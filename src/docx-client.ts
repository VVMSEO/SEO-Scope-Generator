...
import { Project, GeneratedScopeResult } from "./types.js";

export function generateDocxDocument(project: Project, scope: GeneratedScopeResult): Document {
  return new Document({
    sections: [{
      properties: {},
      children: [
...
export async function downloadClientDocx(project: Project, scope: GeneratedScopeResult): Promise<void> {
  const doc = generateDocxDocument(project, scope);
  const blob = await Packer.toBlob(doc);
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  const safeTitle = project.name.replace(/[\\/*?:|"<>\s]+/g, "_");
  link.download = `sow_month_${scope.month}_${safeTitle}.docx`;
